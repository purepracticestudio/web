  ...
  */
  ...
  $('.submit').on('click',function(e){
    ...
    var data = JSON.stringify({ 'CustomerName': $(':text[name="name"]').val(),
            ...
            'HouseStatus': '4',
            'Note': '客廳知識頁專案來案，請與客戶聯繫後更新資料。', //'專案案來案，請與客戶聯繫後更新資料。'
            'ServiceItems': [],
            'TitleName':'客廳這樣設計太舒適！打造待客、起居的完美小天地｜信義居家', //專案的title
            'TitleUrl': 'https://living.sinyi.com.tw/events/livingroom-design/'   //專案的網址命名
    });
...
$("#NAVNAV1").on("click", function(e){
  $('html, body').animate({
    scrollTop: $("#choosepings").offset().top  // 只需修改此處
  }, 750);  // 750是滑動的時間，單位為毫秒
  e.preventDefault();
});

$("#NAVNAV2").on("click", function(e){
  $('html, body').animate({
    scrollTop: $("#point").offset().top  // 只需修改此處
  }, 750);  // 750是滑動的時間，單位為毫秒
  e.preventDefault();
});


$("#NAVNAV3").on("click", function(e){
  $('html, body').animate({
    scrollTop: $("#recommend").offset().top  // 只需修改此處
  }, 750);  // 750是滑動的時間，單位為毫秒
  e.preventDefault();
});


$("#NAVNAV4").on("click", function(e){
  $('html, body').animate({
    scrollTop: $("#article").offset().top  // 只需修改此處
  }, 750);  // 750是滑動的時間，單位為毫秒
  e.preventDefault();
});
